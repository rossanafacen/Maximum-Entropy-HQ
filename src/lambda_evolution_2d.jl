...
function pr2_integrand(ur,T,fug,pt,m,eta,phi,etap,phip,mult_diff,mult_n)
    ut = sqrt(1+ur^2)
    mt = sqrt(pt^2+m^2)
    udotp= -ut*mt*cosh(etap-eta)+ur*pt*cos(phip-phi)
    pr = pt*cos(phip-phi)
    return -pr^2/udotp*f_ME(ur,T,fug,pt,m,eta,phi,etap,phip,mult_n,mult_diff)/(2*pi)^3*pt
end

"""Evolution of the Lagrange multiplier in time"""
function lambda_evolution(res, eos, dnu, dalpha; m = 1.5)
    eta = 0
    phi = 0
    ...
    for t in (18)
        for i in 1:150
            ...
            pr2_integral, err = hcubature( b->2*fmGeV^3*pr2_integrand(ur,T,fug,b[3],m,eta,phi,b[1],b[2],lr[t,i],ln[t,i]),(etap_min,phip_min,pt_min),(etap_max,phip_max,pt_max))
