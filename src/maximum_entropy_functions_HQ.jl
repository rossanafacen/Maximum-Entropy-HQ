...
function charm_density_integrand(T,ur,fug,eta,phi,etap,phip,pt,lm::Lagr_Multiplier_1D; m = 1.5)
    ut = sqrt(1+ur^2)
    mt = sqrt(pt^2+m^2)
    udotp= -ut*mt*cosh(etap-eta)+ur*pt*cos(phip-phi)
...
function charm_diff_current_integrand(T,ur,fug,eta,phi,etap,phip,pt,lm::Lagr_Multiplier_1D; m = 1.5)
...
function lagrangian_multipliers_system(T,ur,fug,nur,unknown;m=1.5)
