...
"""
function f_ME(ur,eta,phi,etap,phip,pt,lm::Lagr_Multiplier_LF;m)
    ut = sqrt(1+ur^2)
    mt = sqrt(pt^2+m^2)
...
"""
function energy_integrand(ur,eta,phi,etap,phip,pt,lm::Lagr_Multiplier_LF; m)
    ut = sqrt(1+ur^2)
    mt = sqrt(pt^2+m^2)
    udotp= -ut*mt*cosh(etap-eta)+ur*pt*cos(phip-phi)
...
"""
function energy_density(ur,lm::Lagr_Multiplier_LF;m,etap_min=0,etap_max=10,phip_min=0,phip_max=2pi,pt_min=0.,pt_max=8.0,rtol=10E-6)
    eta = 0
    phi = 0 
...
function lagrangian_multipliers_system_LF_1D(ur,epsilon,unknown; m)
...
"""
function lambda_res_1d_LF(result, discretization::CartesianDiscretization,fluidproperty,t; m, guess_n = 0., guess_nu = 0., diff_n = 0.0,diff_nu = 0.0)  
    lambda_array = []
    ...
    for i in 2:lastindex(discretization.grid)-1
        ...
        _f(unknown,p) = lagrangian_multipliers_system_LF_1D(ur,epsilon,unknown; m = m)
